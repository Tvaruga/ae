...
if(!empty($arResult['ITEMS'])):
	...
	$delimiter = ($delimiter>1)?ceil($delimiter/3):$delimiter;

	?><div class="cell s-4"><div class="c-list t-<?=$arResult['PERSON_MODE']?3:2?>"><?
	foreach($arResult['ITEMS'] as $key=>$arItem):
		if($key!=0 && $key%$delimiter==0)
			echo '</div></div><div class="cell s-4"><div class="c-list t-'.($arResult['PERSON_MODE']?3:2).'">';
		...
		if($arResult['PERSON_MODE']){
			...
			?><div class="c-list__header"><?
				...
				?><div class="c-list__holder"><?
					if($arParams['DISPLAY_DATE']!="N" && !empty($arItem['ACTIVE_FROM'])):
						...
						if($date!==date('j')||$month!=date('n')||$year!=date('Y')):
							?><div class="c-list__day"><?=$day.' '.GetMessage('MONTH_'.$month.'_S').(date('Y')!=$year?' '.$year:'')?></div><?
						endif;
						echo '</div>';
					endif;
			...
			if($arParams['DISPLAY_DATE']!="N" && !empty($arItem['ACTIVE_FROM'])):
				...
				?><div class="c-list__time"><?=date('G:i',$timestamp)?></div><?
				if($date!==date('j')||$month!=date('n')||$year!=date('Y')):
					?><div class="c-list__date"><?=$day.' '.GetMessage('MONTH_'.$month.'_S').(date('Y')!=$year?' '.$year:'')?></div><?
				endif;
			endif;
